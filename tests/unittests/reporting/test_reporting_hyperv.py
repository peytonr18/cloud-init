...
class TestKvpEncoding:
    def test_encode_decode(self):
        ...
        assert len(data) == kvp_reporting.HV_KVP_RECORD_SIZE
        decoded_kvp = kvp_reporting._decode_kvp_item(data)
        assert kvp == decoded_kvp
...
class TestKvpReporter:
    ...
    def test_events_with_higher_incarnation_not_over_written(
        self, kvp_file_path, reporter
    ):
        assert 0 == len(list(reporter._iterate_kvps(0)))
        ...
        reporter.q.join()
        assert 2 == len(list(reporter._iterate_kvps(0)))

        reporter3 = HyperVKvpReportingHandler(kvp_file_path=kvp_file_path)
        reporter3.incarnation_no = reporter.incarnation_no - 1
        reporter3.publish_event(
            events.ReportingEvent("foo", "name3", "description")
        )
        reporter3.q.join()
        assert 3 == len(list(reporter3._iterate_kvps(0)))
    ...
    def test_not_truncate_kvp_file_modified_after_boot(
        self, kvp_file_path, reporter
    ):
        with open(kvp_file_path, "wb+") as f:
            kvp = {"key": "key1", "value": "value1"}
            data = struct.pack(
                "%ds%ds"
                % (
            ...
            f.write(data)
        cur_time = time.time()
        os.utime(kvp_file_path, (cur_time, cur_time))
    ...
    def test_truncate_stale_kvp_file(self, kvp_file_path, reporter):
        with open(kvp_file_path, "wb+") as f:
            kvp = {"key": "key1", "value": "value1"}
            data = struct.pack(
                "%ds%ds"
        ...
        os.utime(kvp_file_path, (1000000, 1000000))
    ...
    def test_report_diagnostic_event_without_logger_func(self, reporter):
    ...
    def test_report_diagnostic_event_with_logger_func(self, reporter):
    ...
    def test_report_compressed_event(self, reporter):
        try:
    ...
    def validate_compressed_kvps(self, reporter, count, values):
        ...
        for i in range(len(kvps)):
            ...
            assert compressed_count < len(values)
            assert evt_data == values[compressed_count]
            assert evt_encoding == "gz+b64"
            compressed_count += 1
        assert compressed_count == count

    def test_unique_kvp_key(self, reporter):
